...
/**
 * Static functions used in the ltfw onclick import plugin
 */
class Long_Toolkit_Import_Helpers {
	...
	public static function download_import_files( $import_file_info, $import_index ) {
		...
		$downloader = new Long_Toolkit_Import_Downloader();
		...
		if ( !empty( $import_file_info['content_url'] ) ) {
			if ( file_exists( $import_file_info['content_url'] ) ) {
				...
				$content_filename = apply_filters( 'long_toolkit_importer_downloaded_content_file_prefix', 'demo-content-import-file_' ) . self::$demo_import_start_time . apply_filters( 'long_toolkit_importer_downloaded_content_file_ext', '.xml' );
			...
			return new WP_Error(
					'content_url_not_defined', sprintf(
							__( '"content_url" for %s%s%s are not defined!', 'long-toolkit' ), '<strong>', $import_file_info['name'], '</strong>'
					)
			);
		}
		...
		if ( !empty( $import_file_info['widget_url'] ) ) {
			...
			if ( file_exists( $import_file_info['widget_url'] ) ) {
				...
				$widget_filename = apply_filters( 'long_toolkit_importer_downloaded_widgets_file_prefix', 'demo-widgets-import-file_' ) . self::$demo_import_start_time . apply_filters( 'long_toolkit_importer_downloaded_widgets_file_ext', '.wie' );
		...
		if ( !empty( $import_file_info['customizer_url'] ) ) {
			...
			if ( file_exists( $import_file_info['customizer_url'] ) ) {
				...
				$customizer_filename = apply_filters( 'long_toolkit_importer_downloaded_customizer_file_prefix', 'demo-customizer-import-file_' ) . self::$demo_import_start_time . apply_filters( 'long_toolkit_importer_downloaded_customizer_file_ext', '.dat' );
		...
		if ( !$wp_filesystem->put_contents( $file_path, $content ) ) {
			return new WP_Error(
					'failed_writing_file_to_server', sprintf(
							__( 'An error occurred while writing file to your server! Tried to write a file to: %s%s.', 'long-toolkit' ), '<br>', $file_path
					)
			);
		}
	...
	public static function verify_ajax_call() {
		...
		if ( !current_user_can( 'import' ) ) {
			wp_die(
					sprintf(
							__( '%sYour user role isn\'t high enough. You don\'t have permission to import demo data.%s', 'long-toolkit' ), '<div class="notice  notice-error"><p>', '</p></div>'
					)
			);
		}
	}
	...
	public static function process_uploaded_files( $uploaded_files ) {
		...
		if ( isset( $_FILES['content_file'] ) ) {
			if ( self::file_validate( 'content_file', '.xml' ) ) {
				...
				$error_text = esc_html__( 'Please upload XML file for content import.', 'long-toolkit' );
		...
		if ( isset( $_FILES['widget_file'] ) ) {
			if ( self::file_validate( 'widget_file', array( '.wie, .json' ) ) ) {
				...
				// Send an AJAX response with the error.
				$error_text = esc_html__( 'Please upload WIE or JSON file for widget import.', 'long-toolkit' );
				// Send JSON Error response to the AJAX call.
				wp_send_json( $error_text );
			}
		}
		...
		if ( isset( $_FILES['customizer_file'] ) ) {
			if ( self::file_validate( 'customizer_file', '.dat' ) ) {
				...
				// Send an AJAX response with the error.
				$error_text = esc_html__( 'Please upload DAT file for customizer import.', 'long-toolkit' );
				// Send JSON Error response to the AJAX call.
				wp_send_json( $error_text );
			}
		}
	...
	public static function set_demo_import_start_time() {
		self::$demo_import_start_time = date( apply_filters( 'long_toolkit_importer_date_format_for_file_names', 'Y-m-d__H-i-s' ) );
	}
	...
	private static function check_wp_filesystem_credentials() {
		...
		if ( !( 'direct' === get_filesystem_method() ) ) {
			return new WP_Error(
					'no_direct_file_access', sprintf(
							__( 'This WordPress page does not have %sdirect%s write file access. This plugin needs it in order to save the demo import xml file to the upload directory of your site. You can change this setting with these instructions: %s.', 'long-toolkit' ), '<strong>', '</strong>', '<a href="http://gregorcapuder.com/wordpress-how-to-set-direct-filesystem-method/" target="_blank">How to set <strong>direct</strong> filesystem method</a>'
					)
			);
		}

		// Get import page settings.
		$import_page_setup = apply_filters( 'long_toolkit_importer_page_setup', array(
			'parent_slug' => 'admin.php',
			'id' => 'long_toolkit_importer',
				)
		);
		...
		if ( false === ( $creds = request_filesystem_credentials( $demo_import_page_url, '', false, false, null ) ) ) {
			return new WP_error(
					'filesystem_credentials_could_not_be_retrieved', __( 'An error occurred while retrieving reading/writing permissions to your server (could not retrieve WP filesystem credentials)!', 'long-toolkit' )
			);
		}
		...
		if ( !WP_Filesystem( $creds ) ) {
			return new WP_Error(
					'wrong_login_credentials', __( 'Your WordPress login credentials don\'t allow to use WP_Filesystem!', 'long-toolkit' )
			);
		}
	 ...
	 * @since 1.0
	 */
	public static function long_toolkit_importer_sse( $id = 'print', $message = '', $progress = 0 ) {
