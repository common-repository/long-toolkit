 ...
 * Import customizer
 */
class Long_Toolkit_Importer_Customizer
{
	...
	public function import()
	{
		...
		if ( !file_exists( $this->file ) ) {
			throw new Exception( __( 'Customize file could not be found.', 'long-toolkit' ) );
		}
		...
		if ( !empty( $data ) && is_array( $data ) ) {
			...
			if ( 'array' != gettype( $data ) ) {
				$error = __( 'Error importing settings! Please check that you uploaded a customizer export file.', 'long-toolkit' );
				return;
			}
			...
			if ( $data[ 'template' ] != $template ) {
				$error = __( 'Error importing settings! The settings you uploaded are not for the current theme.', 'long-toolkit' );
				return;
			}
			...
			if ( isset( $data[ 'options' ] ) ) {
				...
				foreach ( $data[ 'options' ] as $option_key => $option_value ) {
					...
					$customize_setting = new Long_Toolkit_Importer_Customize_Setting( $wp_customize, $option_key, array(
						'default'    => '',
						'type'       => 'option', //option
		...
		do_action( 'long_toolkit_importer_customize_end', $data );
	}
	...
	private function import_images( $mods )
	{
		...
		$use_defaulong_toolkit_image = apply_filters( 'long_toolkit_importer_customizer_enable_defaulong_toolkit_image', false);
		$image_url = apply_filters( 'long_toolkit_importer_customzer_defaulong_toolkit_image_url', '' );

		foreach ( $mods as $key => $val ) {

			if ( $this->is_image( $val ) ) {

				if ( $use_defaulong_toolkit_image && !empty( $image_url ) ) {
					$val = $image_url;
				}
