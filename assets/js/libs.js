...
jQuery(function ($) {
    ...
    $.fn.ltfwImagePicker = function () {
        ...
        $document.on('click', '.long-toolkit-image_picker .add_images', function (e) {

            e.preventDefault();
            var $this = $(this);
            var $field = $this.closest('.long-toolkit-image_picker');
            ...
            file_frames[$field.attr('id')].on('select', function () {
                ...
                selection.map(function (attachment) {
                    ...
                    if (ids == '' || $.inArray(attachment.id.toString(), ids) === -1) {
                        ...
                        src = src.replace(long_toolkit_var.upload_url, '');
        ...
        $document.on('click', '.long-toolkit-image_picker .remove', function (e) {
            e.preventDefault();
            var $this = $(this);
            var $input = $this.closest('.long-toolkit-image_picker').find('input[type="hidden"]');
            var ids = $input.val();
        ...
        if ($.fn.sortable) {
            $('.long-toolkit-image_picker .image_list').sortable({
                stop: function (e, ui) {
                    var ids = [];
                    ...
                    $list.closest('.long-toolkit-image_picker').find('input[type="hidden"]').val(ids);
                }
            });
        }
    }

    $.fn.ltfwLink = function () {

        $document.on('click', '.long-toolkit-link .link_button', function (e) {

            e.preventDefault();
            var $block, $input, $url_label, $title_label, value_object, $link_submit, $long_toolkit_link_submit, $long_toolkit_link_nofollow, dialog;
            $block = $(this).closest(".long-toolkit-link");
            $input = $block.find("input.long_toolkit_value");
            $url_label = $block.find(".url-label");
            $title_label = $block.find(".title-label");
            value_object = $input.data("json");
            $link_submit = $("#wp-link-submit");
            $long_toolkit_link_submit = $('<input type="button" name="long_toolkit_link-submit" id="long_toolkit_link-submit" class="button-primary" value="Set Link">');
            $link_submit.hide();
            $("#long_toolkit_link-submit").remove();
            $long_toolkit_link_submit.insertBefore($link_submit);
            $long_toolkit_link_nofollow = $('<div class="link-target long-toolkit-link-nofollow"><label><span></span> <input type="checkbox" id="long-toolkit-link-nofollow"> Add nofollow option to link</label></div>');
            $("#link-options .long-toolkit-link-nofollow").remove();
            $long_toolkit_link_nofollow.insertAfter($("#link-options .link-target"));
            setTimeout(function () {
                var currentHeight = $("#most-recent-results").css("top");
                $("#most-recent-results").css("top", parseInt(currentHeight) + $long_toolkit_link_nofollow.height())
            }, 200);
            ...
            if ($("#long-toolkit-link-nofollow").length) {
                $("#long-toolkit-link-nofollow").prop("checked", value_object.rel.length);
            }


            $long_toolkit_link_submit.unbind("click.ltfwLink").bind("click.ltfwLink", function (e) {
                ...
                options.rel = $("#long-toolkit-link-nofollow")[0].checked ? "nofollow" : "";
                ...
                $link_submit.show();
                $long_toolkit_link_submit.unbind("click.ltfwLink");
                $long_toolkit_link_submit.remove();
                $("#wp-link-cancel").unbind("click.ltfwLink");
                $checkbox.attr("checked", false);
                $("#most-recent-results").css("top", "");
                $("#long-toolkit-link-nofollow").attr("checked", false);
                return false;
            });
            $("#wp-link-cancel").unbind("click.ltfwLink").bind("click.ltfwLink", function (e) {
                e.preventDefault();
                dialog.close('noReset');
                $long_toolkit_link_submit.unbind("click.ltfwLink");
                $long_toolkit_link_submit.remove();
                $("#wp-link-cancel").unbind("click.ltfwLink");
                $("#wp-link-close").unbind("click.ltfwCloseLink");
                window.wpLink.textarea = "";
                return false;
            });
            $('#wp-link-close').unbind('click').bind('click.ltfwCloseLink', function (e) {
                e.preventDefault();
                dialog.close('noReset');
                $long_toolkit_link_submit.unbind("click.ltfwLink");
                $long_toolkit_link_submit.remove();
            ...
            return this.each(function (index, item) {
                ...
                if (!item.id.includes('__i__')) {//check wp version < 4.7
                    ...
                    map.onDragMarker = function (res) {
                        ...
                        $this.find('input.long_toolkit_value').val(string).change();
                    }

                    map.onZoomChanged = function (res) {

                        map.zoom = map.map.getZoom();
                        var data = $this.find('input.long_toolkit_value').val();
                        if ($.trim(data) != '') {
                            data = data.split('|');
    ...
    $.fn.ltfwRepeater = function () {
                            ...
                            $.each(attach_images, function (index, image) {
                                ...
                                if (image.length === 2) {
                                    image_item += '<li class="added" data-id="' + image[0] + '">\n\
                                                    <div class="inner">\n\
                                                        <img alt="" src="' + long_toolkit_var.upload_url + decodeURIComponent(image[1]) + '"/>\n\
                                                    </div>\n\
                                                    <a href="#" class="remove"></a>\n\
                                                </li>';
                        ...
                        if (typeof $select.attr('multiple') != 'undefined') {
                            $select.selectize({
                                ...
                                onChange: function (value) {
                                    var $input = $(this)[0].$wrapper.closest('.repeater-col-field').find('input.long_toolkit_value');
         ...
         *Edit
         */
        $document.on('click', '.long-toolkit-repeater [data-repeater-edit], .long-toolkit-repeater .long-toolkit-widget-title h4', function (e) {

            var $parent = $(this).closest(".long-toolkit-widget");

            if ($parent.hasClass('open')) {
                $parent.find('.long-toolkit-widget-inside').slideUp('fast', function () {
                    $parent.removeClass('open');
                });

            } else {

                $parent.find('.long-toolkit-widget-inside').slideDown('fast', function () {
                    $parent.addClass('open');
                    $document.trigger('long-toolkit-repeater-item-opened', [$parent]);
                });
            }
    ...
    $.fn.ltfwTypography = function () {
        ...
        var font_changed = function ($wrapper, data, data2) {
            ...
            if (data.variants != '') {
                ...
                var options = [];
                var _variants = long_toolkit_var.variants;
            ...
            if (data.subsets != '') {
                ...
                var options = [];
                var _subsets = long_toolkit_var.subsets;
        ...
        $typography.find('.variants select').selectize({
            ...
            onChange: function (value) {
                if (!is_font_changed) {
                    var $field = $(this)[0].$wrapper.closest('.long-toolkit-typography');
                    ...
                    if (text != '') {
                        ...
                        if (_typography_data.hasOwnProperty('variants')) {
                            ...
                            $field.find('.long_toolkit_value').val(val).change();
        ...
        $typo_font_family.selectize({
            ...
            searchField: "label",
            create: false,
            options: long_toolkit_var.fonts,
            render: {
        ...
        $typography.on('change', '.subrow input, .subrow select', function (e) {
            ...
            var $field = $this.closest('.long-toolkit-typography');
...
//            },
            load: function (query, callback) {
                ...
                var $container = $(this)[0].$wrapper.closest('.long-toolkit-field');
                ...
                $.ajax({
                    ...
                    type: 'GET',
                    data: {
                        action: 'long_toolkit_autocomplete_' + type,
                        types: values,
                        s: query
                    },
                    error: function () {
     ...
     * @since 1.0.4
     */
    $.fn.ltfwUpload = function () {

        var $field = $(this);

        $field.on('change', '.long_toolkit_value', function (e) {
            var $input = $(this);
            var $tr = $input.closest('.long-toolkit-field').find('tbody tr');
            var arr = [];
            $tr.each(function () {
                var $this = $(this);
                arr.push({
        ...
        $field.on('change', '[data-name="name"]', function (e) {
            $(this).closest('.long-toolkit-field').find('.long_toolkit_value').change();
            e.preventDefault();
        });
        ...
        $field.on('click', 'table a.delete', function (e) {
            ...
            var $this = $(this);
            var $tr = $this.closest('.long-toolkit-field').find('tbody tr');
            var $input = $this.closest('.long-toolkit-field').find('.long_toolkit_value');
        ...
        $field.find('tbody').sortable({
            ...
            opacity: 0.65,
            stop: function (e, ui) {
                $(e.target).closest('.long-toolkit-field').find('.long_toolkit_value').change();
            }
        });
        ...
        $(document.body).on('click', '.long-toolkit-upload .upload_file', function (event) {
            var $el = $(this);
            var $container = $el.closest('.long-toolkit-field');
            ...
            file_frames[$container.attr('id')] = wp.media.frames.long_toolkit_file = wp.media({
                ...
                selection.map(function (attachment) {
                    ...
                    if ($.trim(types) != '' && types.indexOf(attachment.mime) !== -1) {
                        ...
                        if (attachment.url) {
                            ...
                            $container.find('.long_toolkit_value').change();
                         ...
                         */
                        alert(long_toolkit_var.upload_invalid_mime);
                    }
    ...
    $.fn.ltfwTextfields = function () {
        ...
        var update_value = function (e, val) {
            var value = [];
            var $parent = $(e.target).closest('.long-toolkit-textfield');
            var $items = $parent.find('li');
            var $input = $parent.find('.long_toolkit_value');
        ...
        $el.on('click', '.addnew', function (e) {
            var $this = $(this);
            var $list = $this.closest('.long-toolkit-textfield').find('ul');
    ...
    $.fn.initWidgetDependency = function () {
        ...
        $(this).each(function () {
            ...
            if (dependency != undefined && _.isObject(dependency)) {
                ...
                var $slave = $('#' + param_name + ".long_toolkit_value");
    ...
    $.fn.initRepeaterDependency = function () {
        ...
        $(this).each(function () {
            ...
            var index = $(this).closest('.long_toolkit_repeater__item').index();
            var repeater_id = $(this).closest('.long-toolkit-repeater').attr('data-name') + '-' + index + '-';
            ...
            if (dependency != undefined && _.isObject(dependency)) {
                ...
                var $slave = $('#' + repeater_id + param_name + ".long_toolkit_value");
...
var Long_Toolkit_Repeater_Item = function ($list, $item, isAdd) {
    'use strict';
    ...
    this.control = $list.prev('.long_toolkit_value');

    this.container.on('keyup change', '.long_toolkit_value', function (e) {
        self.setValues();
    });
    ...
    this.setValues = function () {
        ...
        self.list.find('[data-repeater-item]').each(function () {
            ...
            jQuery(this).find('.long_toolkit_value').each(function () {
