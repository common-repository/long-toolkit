 ...
 */
function long_toolkit_form_link( $settings, $value ) {
	 ...
	 */
	$css_class = 'long-toolkit-field long-toolkit-link';
	...
	$link = long_toolkit_build_link( $value );

	$json_value = htmlentities( json_encode( $link ), ENT_QUOTES, 'utf-8' );

	$input_value = htmlentities( $value, ENT_QUOTES, 'utf-8' );
	?>
	<div class="<?php echo esc_attr( $css_class ) ?>" id="long-toolkit-link-<?php echo esc_attr( uniqid() ) ?>">

		<?php printf( '<input type="hidden" class="long_toolkit_value" value="%1$s" data-json="%2$s" %3$s/>', $input_value, $json_value, implode( ' ', $attrs ) ); ?>

		<a href="#" class="button link_button"><?php echo esc_attr__( 'Select URL', 'long-toolkit' ) ?></a> 
		<span class="group_title">
			<span class="link_label_title link_label"><?php echo esc_attr__( 'Link Text:', 'long-toolkit' ) ?></span> 
			<span class="title-label"><?php echo isset( $link['title'] ) ? esc_attr( $link['title'] ) : ''; ?></span> 
		</span>
		<span class="group_url">
			<span class="link_label"><?php echo esc_attr__( 'URL:', 'long-toolkit' ) ?></span> 
 ...
 * @return void
 */
function long_toolkit_link_editor_hidden() {
