 ...
 */
function long_toolkit_form_image_select( $settings, $value ) {
	...
	if ( is_array( $settings['options'] ) ) {
		...
		$output .= sprintf( '<div class="long-toolkit-field long-toolkit-image_select">' );

		foreach ( $settings['options'] as $radio_key => $image_url ) {

			$checked = $radio_key === $value ? 'checked' : '';
			
			$output .= sprintf( '<label><input class="long_toolkit_value" type="radio" value="%1$s" %3$s %4$s/><span><img alt="%1$s" src="%2$s" %5$s/></span></label>', $radio_key, esc_url( $image_url ), $checked, implode( ' ', $attrs ), $image_size );
