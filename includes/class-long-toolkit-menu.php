...
if ( !class_exists( 'Long_Toolkit_Menu_Field' ) ) {

	class Long_Toolkit_Menu_Field {
		...
		public function __construct() {
			...
			$this->fields = apply_filters( 'long_toolkit_menu_fields', array() );
		...
		public function form( $id, $item, $depth, $args ) {
			...
			foreach ( $this->fields as $field ) {
				...
				if ( $field['type'] == 'repeater' ) {
					printf( '<p class="description "><i class="fa fa-warning"></i> %s</p>', __( '<strong>Repeater field</strong> was not supported in menu item.', 'long-toolkit' ) );
				} else {
		...
		private function field_render( $field, $post_id ) {
			...
			if ( function_exists( "long_toolkit_form_{$field['type']}" ) ) {
				...
				$callback = call_user_func( "long_toolkit_form_{$field['type']}", $field, $field['value'] );
				...
				if ( $field['type'] == 'checkbox' && !$multiple ) {
					...
					$field_output = sprintf( '<div class="long_toolkit_form_row long_toolkit_form_row--checkbox" %4$s><div class="col-field">%2$s %1$s %3$s</div></div>', $lable, $callback, $desc, $attrs );
				} else {
		...
		public function edit_menu() {
			if ( class_exists( 'Long_Toolkit_Walker_Nav_Menu_Edit' ) ) {
				return 'Long_Toolkit_Walker_Nav_Menu_Edit';
			}
		}
...
if ( !class_exists( 'Long_Toolkit_Walker_Nav_Menu_Edit' ) ) {
	...
	class Long_Toolkit_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit {
