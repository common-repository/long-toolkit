...
/**
 * Long_Toolkit_Widget abstract class
 */
abstract class Long_Toolkit_Widget extends WP_Widget {
	...
	public function registered_fields( $hook ) {
		if ( $hook == 'widgets.php' ) {
			...
			if ( empty( $long_toolkit_registered_fields ) ) {
				$long_toolkit_registered_fields = array();
			}
			...
			foreach ( $this->fields as $key => $field ) {
				...
				if ( !empty( $field['fields'] ) ) {
					foreach ( $field['fields'] as $k => $child ) {
						$long_toolkit_registered_fields[] = $child['type'];
					}
				}
			}
		}
	...
	public function get_cached_widget( $args ) {
		...
		$cache = wp_cache_get( apply_filters( 'long_toolkit_cached_widget_id', $this->widget_id ), 'widget' );
	...
	public function cache_widget( $args, $content ) {
		wp_cache_set( apply_filters( 'long_toolkit_cached_widget_id', $this->widget_id ), array( $args['widget_id'] => $content ), 'widget' );
	...
	public function flush_widget_cache() {
		wp_cache_delete( apply_filters( 'long_toolkit_cached_widget_id', $this->widget_id ), 'widget' );
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		foreach ( $this->fields as $field ) {
			...
			if ( isset( $field['sanitize_callback'] ) && function_exists( $field['sanitize_callback'] ) ) {
				...
				if ( $field['type'] == 'file' ) {
					$value = long_toolkit_sanitize_upload( $input_value );
			 ...
			 */
			$value = apply_filters( "long_toolkit_sanitize_field_{$field['type']}", $value, $new_instance[$field['name']] );
	...
	public function form( $instance ) {
		...
		$this->field_wrapper = '<div class="long_toolkit_form_row" %3$s><div class="col-label">%1$s</div><div class="col-field">%2$s</div></div>';

		$output = '';

		foreach ( $this->fields as $field ) {
			/**
			 * @var $defaulong_toolkit_value Default Value
			 */
			$defaulong_toolkit_value = isset( $field['value'] ) ? $field['value'] : '';

			/**
			 * @var $defaulong_toolkit_value Default Value
			 * If you used to use `std` as a `default value`, please use it :)
			 */
			$defaulong_toolkit_value = isset( $field['std'] ) ? $field['std'] : $defaulong_toolkit_value;

			/**
			 * @var $value Field value
			 */
			$value = isset( $instance[$field['name']] ) ? $instance[$field['name']] : $defaulong_toolkit_value;
			 ...
			 */
			do_action( 'long_toolkit_before_render_field_' . $field['type'], $field );
			...
			if ( function_exists( "long_toolkit_form_{$field['type']}" ) ) {
				...
				if ( $field['type'] == 'checkbox' && !$multiple ) {
					...
					$checkbox_wrapper = '<div class="long_toolkit_form_row" %4$s><div class="col-field">%2$s %1$s %3$s</div></div>';

					$output .= sprintf( $checkbox_wrapper, $lable, call_user_func( "long_toolkit_form_{$field['type']}", $field, $value ), $desc, $attrs );
				} else {

					$lable = !empty( $field['heading'] ) ? sprintf( '<label for="%1$s">%2$s %3$s</label>', $field['id'], $field['heading'], $required ) : '';

					$output .= sprintf( $this->field_wrapper, $lable, call_user_func( "long_toolkit_form_{$field['type']}", $field, $value ) . $desc, $attrs );
				}
				...
				$field_output = apply_filters( "long_toolkit_form_{$field['type']}", '', $field );
