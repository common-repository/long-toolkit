 ...
 * @version   1.0.0
 */
if ( !class_exists( 'Long_Toolkit_Taxonomy' ) ) {

	/**
	 * Long_Toolkit_Taxonomy Class
	 */
	class Long_Toolkit_Taxonomy {
		...
		public function __construct( $args = array() ) {
			...
			if ( !empty( $args ) ) {
				...
				$defaults = array(
					'id' => 'long_toolkit_termbox',
					'pages' => array(),
					'title' => __( 'Long_Toolkit Termbox', 'long-toolkit' ),
					'fields' => array(
					)
				);
		...
		public function init() {
			...
			$this->field_edit_wrapper = '<tr class="form-field term-group-wrap long_toolkit_form_row" %3$s><th scope="row">%1$s</th><td>%2$s</td></tr>';
			$this->field_add_wrapper = '<div class="long_toolkit_form_row" %3$s><div class="col-label">%1$s</div><div class="col-field">%2$s</div></div>';
		...
		private function pre_output( $term_id = 0 ) {
			...
			if ( count( $this->group_fields ) == 1 && !key( $this->group_fields ) ) {
				...
				if ( !empty( $metabox['manage_box'] ) ) {
					...
					$checkbox = '<input class="long-toolkit-manage_group" type="checkbox" name="' . $metabox['id'] . '" value="1" ' . checked( 1, $value, false ) . '/>';
					$group_label = $checkbox . esc_html__( 'Enable', 'long-toolkit' ) . ' ' . $group_label;
				}
		...
		private function field_render( $field ) {
			...
			if ( function_exists( "long_toolkit_form_{$field['type']}" ) ) {
				...
				$field_output = sprintf( $this->field_wrapper, $lable, call_user_func( "long_toolkit_form_{$field['type']}", $field, $field['value'] ) . $desc, $attrs );
